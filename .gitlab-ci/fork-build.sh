...
meson -Dplatforms=x11,wayland -Ddri-drivers= -Dgallium-drivers=v3d,vc4,swrast -Ddri-search-path=$DRI_DRIVERS_PATH -Dvulkan-drivers= -Dglvnd=false -Dllvm=disabled -Dlibunwind=disabled -Dzstd=enabled --buildtype=release -Db_lto=true -Dgallium-nine=true -Dd3d-drivers-path=/usr/local/lib/d3d $EXTRA_MESON_ARGS
